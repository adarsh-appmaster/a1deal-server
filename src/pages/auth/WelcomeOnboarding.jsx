...
export default function WelcomeOnboarding() {
  ...
  return (
        ...
        <div className="flex items-center gap-2 mb-8 justify-center">
          {STEPS.map((s, i) => (
            <div key={s} className="flex items-center gap-2">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-colors ${i <= step ? 'bg-primary text-white' : 'bg-slate-200 text-slate-400'}`}>
                {i < step ? <span className="material-icons-outlined text-base">check</span> : i + 1}
              </div>
              <span className={`text-xs font-semibold ${i === step ? 'text-primary' : 'text-slate-400'}`}>{s}</span>
              {i < STEPS.length - 1 && <div className={`w-8 h-0.5 rounded-full ${i < step ? 'bg-primary' : 'bg-slate-200'}`} />}
            </div>
          ))}
        </div>
      }
    ...
    >
      ...
      {step === 0 && (
        <>
          <h1 className="font-montserrat font-bold text-2xl text-on-surface mb-1">Welcome to A1 Deal!</h1>
          ...
          <div className="grid grid-cols-2 gap-3">
            {GOALS.map(g => (
              <button
                ...
                className={`text-left p-4 rounded-2xl border-2 transition-all ${goal === g.id ? 'border-primary bg-primary/5' : 'border-slate-200 hover:border-slate-300'}`}
              >
                <span className={`material-icons-outlined text-2xl mb-2 block ${goal === g.id ? 'text-primary' : 'text-slate-400'}`}>{g.icon}</span>
                <p className={`font-semibold text-sm ${goal === g.id ? 'text-primary' : 'text-on-surface'}`}>{g.label}</p>
      ...
      {step === 1 && (
        <>
          <h1 className="font-montserrat font-bold text-2xl text-on-surface mb-1">What's your budget?</h1>
          ...
          <div className="space-y-2">
            {BUDGETS.map(b => (
              <button
                ...
                className={`w-full text-left px-5 py-3.5 rounded-xl border-2 font-semibold text-sm transition-all ${budget === b.id ? 'border-primary bg-primary/5 text-primary' : 'border-slate-200 text-on-surface hover:border-slate-300'}`}
              >
                {b.label}
      ...
      {step === 2 && (
        <>
          <h1 className="font-montserrat font-bold text-2xl text-on-surface mb-1">Preferred cities?</h1>
          ...
          <div className="flex flex-wrap gap-2">
            {CITIES.map(city => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm font-semibold border-2 transition-all ${cities.includes(city) ? 'border-primary bg-primary/5 text-primary' : 'border-slate-200 text-slate-600 hover:border-slate-300'}`}
              >
      ...
      <div className="flex gap-3 mt-8">
        ...
        <button
          ...
          disabled={!canNext}
          className="flex-1 py-3 rounded-xl bg-primary text-white font-bold text-sm hover:bg-primary-container transition disabled:opacity-40"
        >
