    ...
    <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center px-4">
      <div className="w-full max-w-sm">
        ...
        <div className="bg-slate-800 rounded-3xl p-8 border border-slate-700">
          {/* Icon */}
          <div className="w-14 h-14 rounded-2xl bg-[#4900e5]/20 flex items-center justify-center mx-auto mb-5">
            <span className="material-icons-outlined text-3xl text-[#6236ff]">security</span>
          </div>
            ...
            {code.map((digit, i) => (
              <input
                ...
                className={`w-11 h-12 rounded-xl text-center text-lg font-bold bg-slate-700 border-2 text-white focus:outline-none transition-colors
                  ${digit ? 'border-[#6236ff]' : 'border-slate-600'}
                  ${error ? 'border-rose-500' : 'focus:border-[#6236ff]'}`}
              />
            ))}
          </div>
