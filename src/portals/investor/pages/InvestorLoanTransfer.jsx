...
export default function InvestorLoanTransfer() {
  const { user }               = useAuth();
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-0.5">
          Acquire distressed assets or list your own property for loan transfer.
        </p>
      </div>
        ...
        {TABS.map((t, i) => (
          <button key={t} onClick={() => setTab(i)}
            className={`px-4 py-2 text-sm font-semibold rounded-t-xl transition
              ${tab === i ? 'bg-[#059669] text-white' : 'text-slate-500 hover:text-slate-800'}`}>
            {t}
          </button>
        ))}
      </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
              {properties.map(p => (
                <div key={p._id} className="bg-white rounded-2xl border border-slate-100 hover:shadow-lg transition-shadow overflow-hidden">
                  <div className="bg-gradient-to-br from-emerald-50 to-white px-4 pt-4 pb-3">
                  ...
                  <div className="px-4 py-3 grid grid-cols-2 gap-3 border-b border-slate-50">
                    ...
                    <div>
                      ...
                      <p className="font-bold text-[#059669] text-sm">{p.outstandingLoanAmount ? `₹${(p.outstandingLoanAmount / 100000).toFixed(1)}L` : '—'}</p>
                    </div>
                    <div>
      ...
      {tab === 1 && <LoanTransferMySubmissions accentColor="#059669" />}
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4">
            ...
            <div className="p-6 space-y-4 overflow-y-auto max-h-[75vh]">
              ...
              <div>
                ...
                <textarea rows={3} className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#059669]/30"
              ...
              <div className="flex gap-3">
                ...
                <button onClick={() => handleApply(selected)} disabled={applying}
                  className="flex-1 py-2.5 rounded-xl bg-[#059669] text-white text-sm font-semibold hover:bg-[#047857] disabled:opacity-50">
