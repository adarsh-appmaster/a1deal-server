...
export default function AuctionUnitPropertyListing() {
  ...
  const fetchProperties = useCallback(async (search, type) => {
    ...
    try {
      ...
      const r = await api.get(`/auction-unit-properties/public?${params}`);
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-6">
      ...
      <div>
        <h1 className="font-montserrat font-bold text-2xl text-slate-800">⚖️ Auction Unit Properties</h1>
        <p className="text-slate-500 text-sm mt-1">
          Bank-linked auction inventory with developer-style unit listings.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-slate-400">No auction unit properties found.</div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {properties.map(p => (
            <div key={p._id} onClick={() => navigate(`/buyer/auction-unit-properties/${p._id}`)}
              ...
              <ImageSlider
                ...
                overlay={
                  <>
                    ...
                    {p.bankName && (
                      <span className="absolute top-3 left-3 text-xs font-bold px-2 py-0.5 rounded-full bg-slate-800/80 text-white flex items-center gap-1">
                        <span className="material-icons-outlined text-xs">account_balance</span> {p.bankName}
                      </span>
                    )}
                  </>
                }
              ...
              <div className="p-5 space-y-2">
                ...
                {p.auctionDate && (
                  <p className="text-xs text-amber-700 bg-amber-50 rounded-lg px-2 py-1 inline-block">
                    Auction date: {new Date(p.auctionDate).toLocaleDateString('en-IN')}
                  </p>
                )}
                <div className="flex gap-2 pt-2">
                  ...
                  <button
                    onClick={e => {
                      ...
                      navigate(`/buyer/visit/${p._id}`, {
                        state: { propertyTitle: p.title, city: p.city, area: p.area, propertyModel: 'AuctionUnitProperty' },
                      });
                    }}
                  ...
                  <ShareWhatsappButton property={p} path={`/buyer/auction-unit-properties/${p._id}`} iconOnly className="flex-shrink-0" />
                </div>
              </div>
            </div>
          ))}
      ...
      {enquireProperty && (
        <EnquiryModal
          property={{ ...enquireProperty, _model: 'AuctionUnitProperty' }}
